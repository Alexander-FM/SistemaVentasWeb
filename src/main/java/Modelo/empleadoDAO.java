...
public class empleadoDAO extends Conexion {
  ...
  public Empleado solicitarCambioContrasenia(Empleado emp) throws Exception {
    Empleado usus = null;
    ResultSet rs = null;
    String sql = "SELECT E.userName, E.nombre, E.apellidos FROM Empleado E "
        + "WHERE E.userName = '" + emp.getLogin() + "'"; //El dato que envia es string por eso '""'
    try {
      this.conectar(false);
      rs = this.ejecutarOrdenDatos(sql);
      if (rs.next() == true) {
        usus = new Empleado();
        usus.setLogin(rs.getString("userName"));
        usus.setNombre(rs.getString("nombre") + " " + (rs.getString("apellidos")));
      }
    } catch (Exception e) {
  ...
  public void cambiarContrasenia(Empleado emp) throws Exception {
    String sql;
    sql = "UPDATE Empleado SET clave = ENCRYPTBYPASSPHRASE('password', '" + emp.getClave() + "') "
        + "WHERE userName = '" + emp.getLogin() + "'";
